...
bool Prepared_statement::insert_params(THD *thd, String *query, PS_PARAM *parameters) {
  DBUG_TRACE;
...
bool Prepared_statement::insert_params_from_vars(THD *thd, List<LEX_STRING> &varnames,
                                                 String *query) {
  Item_param **end = m_param_array + m_param_count;
...
bool Prepared_statement::prepare_query(THD *thd) {
  DBUG_TRACE;
  DBUG_ASSERT(m_lex == thd->lex);  // set_n_backup_active_arena() guarantees that
  SELECT_LEX *select_lex = m_lex->select_lex;
  ...
  */
  ...
  if ((sql_command_flags[sql_command] & CF_HAS_RESULT_SET) &&
      !m_lex->is_explain()) {
    SELECT_LEX_UNIT *unit = m_lex->unit;
...
void Prepared_statement::setup_set_params(THD *thd) {
  ...
  */
  ...
  /*
    ...
    of insert_params_from_vars; so, if m_with_log is false, it is
    skipped. Testcase:
...
Prepared_statement::~Prepared_statement() {
  ...
  */
  m_arena.free_items();
  if (m_lex != nullptr) {
    DBUG_ASSERT(m_lex->sphead == nullptr);
    lex_end(m_lex);
    m_lex->destroy();
...
bool Prepared_statement::prepare(THD *thd, const char *query_str,
  ...
  */
  DBUG_ASSERT(m_lex->sphead == nullptr || error != 0);
    ...
    if (reprepare_observer.is_invalidated()) {
      ...
      if (err_seen == ER_PREPARE_FOR_SECONDARY_ENGINE ||
        ...
        DBUG_ASSERT((thd->secondary_engine_optimization() ==
                     Secondary_engine_optimization::PRIMARY_TENTATIVELY) ||
                    err_seen == ER_NEED_REPREPARE);
        DBUG_ASSERT(!m_lex->unit->is_executed());
        thd->clear_error();
        if (err_seen == ER_PREPARE_FOR_SECONDARY_ENGINE)
          thd->set_secondary_engine_optimization(
      ...
      if (error && m_lex->m_sql_cmd != nullptr &&
        ...
        if (!error) {
          ...
          DBUG_ASSERT(!m_lex->m_sql_cmd->using_secondary_storage_engine());
...
bool Prepared_statement::reprepare(THD *thd) {
  ...
  */
  DBUG_ASSERT(m_param_count == copy.m_param_count);
...
bool Prepared_statement::validate_metadata(THD *thd, Prepared_statement *copy) {
  ...
  */
  ...
  if (m_lex->select_lex->num_visible_fields() !=
      copy->m_lex->select_lex->num_visible_fields()) {
