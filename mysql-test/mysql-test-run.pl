...
sub rdrs_stop ($) {
  my $rdrs = shift;
  # todo-ronsql Implement process killing, perhaps using signal USR2 or something for clean exit.
  die "not implemented";
}
...
sub rdrs_start ($$) {
  ...
  # Set RDRS_CONFIG_FILE env var only for this process.
  my $config_file = $rdrs->value('config-file');
  my @rdrs_envs = [ "RDRS_CONFIG_FILE=$config_file" ];
  ...
  if ($wait_for_pid_file &&
    ...
    mtr_error("Failed to start mysqld $name with command $exe");
  }
...
sub rdrs_save_config ($) {
  my $rdrs = shift;
  my $pid_file = $rdrs->value('pid-file'); # todo-ronsql implement in RDRS2.
  my $rest_port = $rdrs->value('port');
  my $mgmds_json = join(',', map {
      my $name = $_->name();
      my $cluster_config_this_mgmd = $config->group("cluster_config.$name");
