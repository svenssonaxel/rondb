...
main(int argc, char** argv)
{
  ...
  // length.
  for (int i=0; i < number_of_keywords_implemented_in_rest_sql; i++)
  {
    auto this_word = keywords_implemented_in_rest_sql[i];
    int len = strlen(this_word.text);
    assert(1 <= len && len <= max_strlen_for_keyword_implemented_in_rest_sql);
    for (int j=0; j < len; j++)
    {
    ...
    if (i > 0)
    {
      auto prev_word = keywords_implemented_in_rest_sql[i-1];
