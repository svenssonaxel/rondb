...
FLEX="$1"
SOURCE_RESTSQLLEXER_L="$2"
TARGET_RESTSQLLEXER_L_HPP="$3"
TARGET_RESTSQLLEXER_L_CPP="$4"

[ -x "$FLEX" ]
[ -f "$SOURCE_RESTSQLLEXER_L" ]

TMPDIR="$(mktemp -d)"
trap 'rm -rf "$TMPDIR"' EXIT

"$FLEX" \
    --header-file="$TMPDIR/RestSQLLexer.l.hpp" \
    --outfile="$TMPDIR/RestSQLLexer.l.with-hold_char.cpp" \
    "$SOURCE_RESTSQLLEXER_L" \
    2>&1 | \
    tee "$TMPDIR/RestSQLLexer.l.err"

# flex has no option to treat warnings as errors, so we use a temporary file for
# that.
if [ -s "$TMPDIR/RestSQLLexer.l.err" ]; then
    echo "flex produced non-empty output on stderr:"
    cat "$TMPDIR/RestSQLLexer.l.err"
    exit 1
fi
...
if grep -qE "yy_hold_char| = '.0';" "$SOURCE_RESTSQLLEXER_L"; then
    echo "Source file must not contain patterns that need to be removed from the generated file:"
    grep -HEn "yy_hold_char| = '.0';" "$SOURCE_RESTSQLLEXER_L"
    exit 1
fi

sed -r "/yy_hold_char/d; / = '.0';/d;" "$TMPDIR/RestSQLLexer.l.with-hold_char.cpp" > "$TMPDIR/RestSQLLexer.l.cpp"

if diff -q "$TMPDIR/RestSQLLexer.l.with-hold_char.cpp" "$TMPDIR/RestSQLLexer.l.cpp"; then
...
if grep -qE "yy_hold_char| = '.0';" "$TMPDIR/RestSQLLexer.l.cpp"; then
    echo "Editing to remove hold_char ineffective."
    grep -HEn "yy_hold_char| = '.0';" "$TMPDIR/RestSQLLexer.l.cpp"
    exit 1
fi

# Success

mv "$TMPDIR/RestSQLLexer.l.hpp" "$TARGET_RESTSQLLEXER_L_HPP"
mv "$TMPDIR/RestSQLLexer.l.cpp" "$TARGET_RESTSQLLEXER_L_CPP"
