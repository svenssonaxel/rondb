...
void SimulatedBlock::setNoSend(Uint32 val) {
#ifdef NDBD_MULTITHREADED
  mt_setNoSend(m_threadId, val);
#endif
}

void SimulatedBlock::setNoSendTmp(Uint32 val)
{
#ifdef NDBD_MULTITHREADED
...
void SimulatedBlock::insert_activate_trp(TrpId trp_id)
{
#ifdef NDBD_MULTITHREADED
...
>>>>>>> MySQL 8.0.36
    ...
    ss = mt_send_remote(m_threadId, &sh, 
// RONDB-624 todo: Glue these lines together ^v
<<<<<<< RonDB // RONDB-624 todo
jobBuffer, &signal->theData[0],
                        recNode, 0);
#else
    TrpId trp_id = 0;
    ss = globalTransporterRegistry.
...
void SimulatedBlock::print_static_distr_info(
  ...
  for (Uint32 ldm = 0; ldm < num_ldm_instances; ldm++) {
    ...
    g_eventLogger->info("LDM Thread %u contains Query Worker", ldm,
...
// RONDB-624 todo: Glue these lines together ^v
||||||| Common ancestor
  ldm, first_query_instance
// RONDB-624 todo: Glue these lines together ^v
=======
   first_query_instance
// RONDB-624 todo: Glue these lines together ^v
>>>>>>> MySQL 8.0.36
 + 1);
  }
...
void SimulatedBlock::print_debug_sched_stats(
  ...
  g_eventLogger->info(
      "LQHKEYREQ LQH: %llu"
                      ", LQHKEYREQ QT: %llu"
      ", LQHKEYREQ RR: %llu",
      handle->m_lqhkeyreq_lqh, handle->m_lqhkeyreq_qt, handle->m_lqhkeyreq_rr);
  g_eventLogger->info(
      "SCAN_FRAGREQ LQH: %llu"
      ", SCAN_FRAGREQ QT: %llu"
                      ", SCAN_FRAGREQ RR: %llu",
  ...
  for (Uint32 j = 1; j <= 8; j++) {
    ...
    g_eventLogger->info("LDM(%u) total_signals: %llu, total_words: %llu, "
                        ...
                        "%llu %llu %llu %llu "
        "%llu %llu %llu %llu ",
        j, *total_signals, *total_words, est_stat[0], est_stat[1], est_stat[2], est_stat[3],
        est_stat[4], est_stat[5], est_stat[6], est_stat[7], est_stat[8],
        est_stat[9], est_stat[10],est_stat[11],
                                         ...
                                         Uint32 ldm_instance_no)
{
  ...
  do
  {
    ...
    if (current_stolen < weight)
    {
      ...
      Uint32 extra = (new_current_stolen <= weight) ?
                      0 : (new_current_stolen - weight);
      rr_info->m_used_weight +=
// RONDB-624 todo: Glue these lines together ^v
<<<<<<< RonDB // RONDB-624 todo
 (load_indicator + extra);
      jamDataDebug(rr_info->m_used_weight);
      jamDataDebug(rr_info->m_total_weight);
// RONDB-624 todo: Glue these lines together ^v
||||||| Common ancestor
 current_stolen_count +
// RONDB-624 todo: Glue these lines together ^v
=======
>>>>>>> MySQL 8.0.36
