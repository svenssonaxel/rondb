...
// RONDB-624 todo: Glue these lines together ^v
<<<<<<< RonDB // RONDB-624 todo
NDB_PARTITION_BALANCE_FOR_RP_BY_LDM_X_8:
// RONDB-624 todo: Glue these lines together ^v
||||||| Common ancestor
req,
// RONDB-624 todo: Glue these lines together ^v
...
// RONDB-624 todo: Glue these lines together ^v
<<<<<<< RonDB // RONDB-624 todo
"NDB_PARTITION_BALANCE_FOR_RA_BY_LDM_X_16";
      default:
        ndbabort();
    }
    return NULL;
 ...
 public:
                              ...
                              RoundRobinInfo *rr_info)
  {
    ...
    for (Uint32 i = 0; i < num_ldm_instances; i++)
    {
      ...
      if (rr_group != in_rr_group)   continue;
      jamDebug();
      jamDataDebug(i+1);
      struct QueryThreadState *q_state = &handle->m_query_state[i];
  ...
  Uint32 getFirstLDMThreadInstance() {
    if (unlikely(globalData.ndbMtLqhThreads == 0)) {
      return globalData.ndbMtMainThreads + globalData.ndbMtTcThreads;
    } else {
...
inline
BlockReference SimulatedBlock::calcTcBlockRef(NodeId aNodeId) {
