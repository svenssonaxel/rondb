...
SET(fatal, NDBD_EXIT_INVALID_CONFIG,
...
// RONDB-624 todo: Glue these lines together ^v
||||||| Common ancestor
fsPath= get_and_validate_path(iter,
                                 CFG_DB_FILESYSTEM_PATH,
                                 "FileSystemPath");
  if (_backupPath)
    free(_backupPath);
...
>>>>>>> MySQL 8.0.36
 =
      get_and_validate_path(iter, CFG_DB_BACKUP_DATADIR,                    globalData.ndbRRGroups);
    }
    else
   "Invalid 
// RONDB-624 todo: Glue these lines together ^v
...
// RONDB-624 todo: Glue these lines together ^v
<<<<<<< RonDB // RONDB-624 todo
globalData.ndbMtReceiveThreads == 0)
...
ndb_mgm_configuration_iterator *Configuration::getOwnConfigIterator()
    const {
...
<<<<<<< RonDB // RONDB-624 todo
  ...
  if (reservedOperations == 0)
  {
     ...
     * non-reserved operation records
// RONDB-624 todo: Glue these lines together ^v
||||||| Common ancestor
                ", still possible to set MaxNoOfLocalScans"
                          " explicitly to go higher"
...
>>>>>>> MySQL 8.0.36
 0)
  {
    reservedTransactions = noOfTransactions 
// RONDB-624 todo: Glue these lines together ^v
<<<<<<< RonDB // RONDB-624 todo
/ 4
// RONDB-624 todo: Glue these lines together ^v
...
// RONDB-624 todo: Glue these lines together ^v
<<<<<<< RonDB // RONDB-624 todo
 DO_DIV(noOfLocalOperations,
// RONDB-624 todo: Glue these lines together ^v
||||||| Common ancestor
 DO_DIV(noOfLocalScanRecords,
=======

>>>>>>> MySQL 8.0.36
 ldmInstances);
  noOfLocalScanRecords = DO_DIV(noOfLocalScanRecords, 
// RONDB-624 todo: Glue these lines together ^v
<<<<<<< RonDB // RONDB-624 todo
ldmInstances);
