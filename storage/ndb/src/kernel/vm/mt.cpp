...
static inline int try_wakeup(struct thr_wait *wait, struct thr_data *thrptr) { return wakeup(wait, thrptr); }
#else
...
static inline int try_wakeup(struct thr_wait *wait, struct thr_data *thr_ptr) {
...
static bool is_main_thread(unsigned thr_no) {
  ...
  unsigned first_recv_thread =
      globalData.ndbMtLqhThreads + globalData.ndbMtRecoverThreads +
...
<<<<<<< RonDB // RONDB-624 todo
||||||| Common ancestor
DEB_MULTI_TRP(("Neighbour(%u) of node %u is
// RONDB-624 todo: Glue these lines together ^v
=======
    ("Neighbour(%u) of node %u is
// RONDB-624 todo: Glue these lines together ^v
>>>>>>> MySQL 8.0.36
...
void thr_send_threads::start_send_threads() {
  for (Uint32 i = 0; i < globalData.ndbMtSendThreads; i++) {
    m_send_threads[i].m_thread = NdbThread_Create(
        mt_send_thread_main, (void **)&m_send_threads[i], 2*1024 * 1024,
 ...
 */
Uint32 thr_send_threads::alert_send_thread(TrpId trp_id, NDB_TICKS now) {
  struct thr_send_thread_instance *send_instance =
...
static void wait_time_tracking(thr_data *selfptr, Uint64 wait_time_in_ns) {
  for (Uint32 i = 0; i < NUM_SPIN_INTERVALS; i++) {
    if (wait_time_in_ns <= selfptr->m_spin_stat.m_spin_interval_ns[i]) {
      ...
      if (unlikely(selfptr->m_spintime_us == 0 && selfptr->m_conf_spintime_us != 0 &&
                   selfptr->m_spin_stat.m_num_waits ==
...
static bool check_recv_yield(thr_data *selfptr,
                             ...
                             Uint64 min_spin_timer_us,  // microseconds
                             Uint32 &num_events, NDB_TICKS start_spin_ticks) {
#ifndef NDB_HAVE_CPU_PAUSE
  /**
   * If cpu_pause() was not implemented, 'min_spin_timer == 0' is enforced,
   * and spin-before-yield is never attempted.
 ...
 */
bool thr_send_threads::assist_send_thread(bool must_send,
  ...
  if (count == 0) {
    if (must_send && assist_send && g_send_threads &&
      ...
      pending_send = g_send_threads->assist_send_thread(
                                         must_send,
                                         num_trps_to_send_to, selfptr->m_thr_no,
                                         now,
...
void mt_init_thr_map() {
  ...
  if (globalData.ndbMtMainThreads == 1) {
    ...
    Uint32 main_thread_no =
        globalData.ndbMtLqhThreads + globalData.ndbMtRecoverThreads + globalData.ndbMtTcThreads;
...
void mt_add_thr_map(Uint32 block, Uint32 instance) {
  ...
  switch (block) {
    ...
    if (receive_threads_only)
    {
      ...
      thr_no += (num_lqh_threads +  num_restore_threads +  (instance - 1));
      }
      break;
    }
    case THRMAN:
    ...
    {
      thr_no +=
          num_lqh_threads +   num_restore_threads +   num_tc_threads +   (instance - 1);
    }
    break;
    default:
      require(false);
...
static void init_thread(thr_data *selfptr) {
  ...
  tmp.appfmt(
      "realtime=%u, spintime=%u, max_send_wakeups=%u"
      ", max_send_wakeups_before_assist=%u",
      selfptr->m_realtime, selfptr->m_conf_spintime_us,
      selfptr->m_max_send_wakeups,
  ...
  while (globalData.theRestartFlag != perform_stop)
  {
    ...
    if (lagging_timers == 0 &&          // 1)
        ...
        (min_spin_timer_us == 0 ||      // 4)
         (sum == 0 && !has_received &&
          check_recv_yield(selfptr, recvdata, min_spin_timer_us,
                           num_events,
                           before))))
    {
      delay = 1; // 1 ms
 ...
 */
static bool handle_full_job_buffers(struct thr_data *selfptr, 
// RONDB-624 todo: Glue these lines together ^v
<<<<<<< RonDB // RONDB-624 todo
Uint32 & send_sum,
...
// RONDB-624 todo: Glue these lines together ^v
=======
a short nap if send failed due to send overload
// RONDB-624 todo: Glue these lines together ^v
>>>>>>> MySQL 8.0.36
,
...
// RONDB-624 todo: Glue these lines together ^v
=======
us = (selfptr->m_node_overload_status >=
// RONDB-624 todo: Glue these lines together ^v
>>>>>>> MySQL 8.0.36
...
: 10 * 1000 * 1000;
    ...
    {
      if (handle_full_job_buffers(selfptr, send_sum, flush_sum)) {
        selfptr->m_stat.m_wait_cnt += waits;
        selfptr->m_stat.m_loop_cnt += loops;
        waits = loops = 0;
        update_rt_config(selfptr, real_time, BlockThread);
...
void get_jbb_estimated_stats(Uint32 block, Uint32 instance,
                        ...
                        Uint64 **total_words, Uint64 **est_stat) {
  struct thr_repository *rep = g_thr_repository;
...
void mt_setConfMaxSendDelay(Uint32 send_delay)
{
...
void mt_setSpintime(Uint32 self, Uint32 new_spintime_us) {
  struct thr_repository *rep = g_thr_repository;
  struct thr_data *selfptr = &rep->m_thread[self];
  /* spintime always 0 on platforms not supporting spin */
  if (!NdbSpin_is_supported()) {
...
Uint32 mt_getMainThrmanInstance() {
  if (globalData.ndbMtMainThreads == 2 || globalData.ndbMtMainThreads == 1)
    ...
    return 1 + globalData.ndbMtLqhThreads + globalData.ndbMtRecoverThreads + globalData.ndbMtTcThreads;
  else
    require(false);
  return 0;
}

void sendlocal(Uint32 self, SignalHeader *s, const Uint32 *data,
               const Uint32 secPtr[3]) {
...
static void thr_init(struct thr_repository *rep, struct thr_data *selfptr,
  ...
  selfptr->m_outstanding_send_wakeups = 0;
#ifdef DEBUG_SCHED_STATS
  for (Uint32 i = 0; i < 16; i++) selfptr->m_jbb_estimated_queue_stats[i] = 0;
...
static void rep_init_crash_mutex(struct thr_repository* rep)
{
...
static
void
rep_init(struct thr_repository *rep, unsigned int cnt,
                     Ndbd_mem_manager *mm) {
  rep->m_mm = mm;
...
Uint32 compute_jb_pages(struct EmulatorData *ed) {
  ...
  if (cnt > NUM_JOB_BUFFERS_PER_THREAD) {
    ...
    tot +=
        num_receive_threads * (cnt - other_threads_no_comm) * thr_job_queue::SIZE;
    /**
     ...
     */
    tot += num_lqh_threads * (num_tc_workers + num_main_threads + 1) *
           thr_job_queue::SIZE;
  ...
  for (thr_no = 0; thr_no < glob_num_threads; thr_no++) {
    ...
    if (thr_no < first_receiver_thread_no) {
      ...
      struct NdbThread *thread_ptr = NdbThread_Create(
          mt_job_thread_main, (void **)(rep->m_thread + thr_no), 2*1024 * 1024,
      ...
      struct NdbThread *thread_ptr =
          NdbThread_Create(mt_receiver_thread_main,
                           (void **)(&rep->m_thread[thr_no]), 2*1024 * 1024,
...
void FastScheduler::dumpSignalMemoryAndJam(Uint32 thr_no, FILE *out) {
  thr_data *selfptr = NDB_THREAD_TLS_THREAD;
...
static bool may_communicate(unsigned from, unsigned to) {
  if (is_main_thread(from) || is_main_thread(to)) {
    ...
    return is_ldm_thread(to) || is_tc_thread(to);  // Cover both SPJs and itself
  } else if (is_ldm_thread(from)) {
    // All LDM threads can communicates with TC-, main- and ldm.
    return is_tc_thread(to) || is_ldm_thread(to) || is_recover_thread(to) || (to == from);
