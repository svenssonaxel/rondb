...
Uint32
Tsman::execFSWRITEREQ(const FsReadWriteReq *req)
...
void Tsman::scan_extent_headers(Signal *signal, Ptr<Datafile> ptr) {
  ...
  for (Uint32 i = 0; i < pages; i++) {
    ...
    for (Uint32 j = 0; j < extents; j++) {
      ...
      Uint32 *ext_fragment_id = page->get_fragment_id(page_extent_no, size, v2);
...
int Tablespace_client::get_tablespace_info(SimulatedBlock *block,
