...
signaldata/CloseComReqConf.hpp>
#include <signaldata/StartOrd
// RONDB-624 todo: Glue these lines together ^v
||||||| Common ancestor
signaldata/StartOrd
// RONDB-624 todo: Glue these lines together ^v
...
// RONDB-624 todo: Glue these lines together ^v
||||||| Common ancestor
DEB_ARBIT(arglist) do { g_eventLogger->info arglist ; } while (0)
#else
// RONDB-624 todo: Glue these lines together ^v
=======
DEB_ARBIT(arglist)       \
...
#else
#define DEB_MULTI_TRP(arglist) do { } while
// RONDB-624 todo: Glue these lines together ^v
=======
MULTI_TRP
...
void Qmgr::execAPI_VERSION_REQ(Signal *signal) {
  ...
  sendSignal(senderRef, GSN_API_VERSION_CONF, signal, siglen,
             JBB);
}
  ...
  if (getNodeState().startLevel < NodeState::SL_STARTED) {
    ...
    switch (aFailCause) {
      case FailRep::ZHEARTBEAT_FAILURE:
        ...
        break;
      default:
        BaseString::snprintf(buf, 100, "Our node failed since Node %d failed", failedNodePtr.i);
    }
    progError(__LINE__, NDBD_EXIT_SR_OTHERNODEFAILED, buf);
  }
...
void Qmgr::execGET_NUM_MULTI_TRP_REQ(Signal *signal) {
  ...
  if (m_initial_set_up_multi_trp_done && nodePtr.p->m_used_num_multi_trps > 1) {
     ...
     */
    jam();
    DEB_MULTI_TRP(("Node %u starting, 
// RONDB-624 todo: Glue these lines together ^v
<<<<<<< RonDB // RONDB-624 todo
prepare switch trp using %u trps",
                   sender_node_id,
  ...
  {
    jam();
    DEB_MULTI_TRP(("Node %u starting, sent
// RONDB-624 todo: Glue these lines together ^v
||||||| Common ancestor
prepare switch trp using %u trps",
                   sender_node_id,
...
it
     ...
     */
    DEB_MULTI_TRP(("Change neighbour node
// RONDB-624 todo: Glue these lines together ^v
||||||| Common ancestor
                                  
// RONDB-624 todo: Glue these lines together ^v
=======
CAST_PTR(ActivateTrpReq,
// RONDB-624 todo: Glue these lines together ^v
>>>>>>> MySQL 8.0.36
 setup for node %u, curr_trp: %u",
                   node_id,
                   current_trp_id));
    startChangeNeighbourNode();
    flush_send_buffers();
...
void Qmgr::check_more_trp_switch_nodes(Signal *signal, bool node_fail) {
  if (!check_all_multi_trp_nodes_connected()) {
    jam();
