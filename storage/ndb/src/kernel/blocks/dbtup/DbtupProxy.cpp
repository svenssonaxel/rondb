 ...
 *rec =
  ...
  {
    jam();
    const Dbtup::Disk_undo::Update_Free
// RONDB-624 todo: Glue these lines together ^v
<<<<<<< RonDB // RONDB-624 todo
_FirstVarPart*
// RONDB-624 todo: Glue these lines together ^v
...
=======
>>>>>>> MySQL 8.0.36
 *rec = (const Dbtup::Disk_undo::Update_Free_FirstVarPart*)ptr;
      undo.m_key.m_file_no = rec->m_file_no_page_idx >> 16;
      undo.m_key.m_page_no = rec->m_page_no;
      undo.m_key.m_page_idx = rec->m_file_no_page_idx & 0xFFFF;
      undo.m_actions |= Proxy_undo::ReadTupPage;
...
// RONDB-624 todo: Glue these lines together ^v
>>>>>>> MySQL 8.0.36
 c_tableRecSize || c_tableRec[tableId] == 0) {
    thrjam(jamBuf);
