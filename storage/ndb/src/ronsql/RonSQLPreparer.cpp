  ...
  for(Uint32 i = 0; i < index_list.count; i++) {
        ...
        col_idx++)
    {
      ...
      for(Uint32 cond_idx = 0;
          cond_idx < m_toplevel_conditions.size() && !(lbound_set && ubound_set);
          cond_idx++)
      {
        ...
        const char* condition_col_name = m_columns[condition_identifier->col_idx].c_str();
  ...
  case T_DATE_SUB:
    {
      ...
      if (amount_ce->op != T_INT) {
        throw runtime_error("INTERVAL literal requires an integer literal amount");
      }
      // Member variables in Interval are unsigned long int or unsigned long long int
      Int64 constant_integer = amount_ce->constant_integer;
