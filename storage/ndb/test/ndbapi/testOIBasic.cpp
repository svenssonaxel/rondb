...
int Val::cmpchars(const Par &par,
              const uchar *buf1,
              uint len1,
              const uchar *buf2,
              uint len2) const {
...
struct Row {
  const Tab &m_tab;
  Val **m_val;
  enum St { StUndef = 0, StDefine = 1, StPrepare = 2, StRead = 3,
    StCommit = 4,
  };
  enum Op {
    OpNone = 0,
...
// RONDB-624 todo: Glue these lines together ^v
||||||| Common ancestor
CHKCON(con.readTuple(par)
// RONDB-624 todo: Glue these lines together ^v
=======
CHKCON(con.m_op->deleteTuple()
// RONDB-624 todo: Glue these lines together ^v
>>>>>>> MySQL 8.0.36
 Delete using Unique Hash 
// RONDB-624 todo: Glue these lines together ^v
<<<<<<< RonDB // RONDB-624 todo
Index
// RONDB-624 todo: Glue these lines together ^v
  ...
  do {
    ...
    if (!(op & Row::OpREAD)) {
      ...
      if (row.m_st == Row::StRead) {
         ...
         * operation if we are in the same 
// RONDB-624 todo: Glue these lines together ^v
<<<<<<< RonDB // RONDB-624 todo
transaction.
||||||| Common ancestor
...
static void calcscanbounds(const Par &par,
               const ITab &itab,
               BSet &bset,
               const Set &set,
               Set &set1) {
...
static int scanreadindex(const Par &par, const ITab &itab, BSet &bset,
  ...
  LL3("scanreadindex " << itab.m_name << " " << bset << " lockmode="
      << par.m_lockmode << " expect=" << set1.count()
                       << " ordered="
      << par.m_ordered << " descending="
                       << par.m_descending << " verify="
      << par.m_verify);
  Set set2(tab, set.m_rows);
  CHK(con.startTransaction() == 0);
...
static int scanreadindexfast(const Par &par,
                  const ITab &itab,
                  const BSet &bset,
                  uint countcheck) {
  Con &con = par.con();
